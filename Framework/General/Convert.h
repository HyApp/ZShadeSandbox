...
namespace ZShadeSandboxGlobal {
	...
	public:
		...
		static float* ConvertStringToFloatArray(string p_pNameToConvert)
		{
			if (p_pNameToConvert.length() == 0) return NULL;
			
			BetterString str(p_pNameToConvert);
			vector<string> splitted = str.split(' ');
			vector<float> floats;
			...
			for (int i = 0; i < splitted.size(); i++)
			{
				...
				{
					floats.push_back(ConvertStringToT<float>(splitted[i]));
				}
			}
