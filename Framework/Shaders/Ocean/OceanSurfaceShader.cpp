...
bool OceanSurfaceShader::Initialize()
{
	...
	*/
	...
	cb_desc.MiscFlags = 0;
	cb_desc.ByteWidth = PAD16(sizeof(cbShadingBuffer));
	m_pD3DSystem->GetDevice11()->CreateBuffer(&cb_desc, NULL, &m_pShadingCB);
	assert(m_pShadingCB);

	cb_desc.ByteWidth = PAD16(sizeof(cbPerCallBuffer));
	m_pD3DSystem->GetDevice11()->CreateBuffer(&cb_desc, NULL, &m_pPerCallCB);
	assert(m_pPerCallCB);

	ClearInputLayout();
	
	SetInputLayoutDesc("OceanSurfaceShader", ZShadeSandboxMesh::VertexLayout::mesh_layout_pos, 1);
	...
	m_pD3DSystem->GetDevice11()->CreateSamplerState(&sam_desc, &g_pCubeSampler);
	assert(g_pCubeSampler);

	sam_desc.Filter = D3D11_FILTER_ANISOTROPIC;
	sam_desc.MaxAnisotropy = 8;
	m_pD3DSystem->GetDevice11()->CreateSamplerState(&sam_desc, &g_pGradientSampler);
	assert(g_pGradientSampler);

	sam_desc.MaxLOD = FLT_MAX;
	sam_desc.MaxAnisotropy = 4;
	m_pD3DSystem->GetDevice11()->CreateSamplerState(&sam_desc, &g_pPerlinSampler);
	...
	m_pD3DSystem->GetDevice11()->CreateSamplerState(&sam_desc, &g_pFresnelSampler);
...
//==============================================================================================================================
void OceanSurfaceShader::BindTextures()
{
	ID3D11ShaderResourceView* vs_srvs[2] = { mTexDisplacement, mTexPerlin };
	m_pD3DSystem->GetDeviceContext()->VSSetShaderResources(0, 2, &vs_srvs[0]);
	
	ID3D11SamplerState* vs_samplers[2] = { m_pD3DSystem->Point(), g_pPerlinSampler };
	m_pD3DSystem->GetDeviceContext()->VSSetSamplers(0, 2, &vs_samplers[0]);
	...
	if (!m_Wireframe)
	{
		...
		m_pD3DSystem->GetDeviceContext()->PSSetShaderResources(0, 9, &ps_srvs[0]);
		
		ID3D11SamplerState* ps_samplers[5] = { g_pPerlinSampler, g_pGradientSampler, g_pFresnelSampler, g_pCubeSampler, m_pD3DSystem->Mirror() };
		m_pD3DSystem->GetDeviceContext()->PSSetSamplers(0, 5, &ps_samplers[0]);
	}
}
//==============================================================================================================================
void OceanSurfaceShader::UnbindTextures()
{
	ID3D11ShaderResourceView* vs_srvs[2] = { NULL, NULL };
	m_pD3DSystem->GetDeviceContext()->VSSetShaderResources(0, 2, &vs_srvs[0]);
	
	ID3D11SamplerState* vs_samplers[2] = { NULL, NULL };
	m_pD3DSystem->GetDeviceContext()->VSSetSamplers(0, 2, &vs_samplers[0]);
	
	if (!m_Wireframe)
	{
		ID3D11ShaderResourceView* ps_srvs[9] = { NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL };
		m_pD3DSystem->GetDeviceContext()->PSSetShaderResources(0, 9, &ps_srvs[0]);
		
		ID3D11SamplerState* ps_samplers[5] = { NULL, NULL, NULL, NULL, NULL };
		m_pD3DSystem->GetDeviceContext()->PSSetSamplers(0, 5, &ps_samplers[0]);
	}
}
//==============================================================================================================================
void OceanSurfaceShader::SetShadingBuffer(OceanParameters ocean_params)
{
	cbShadingBuffer shading_data;
	
	shading_data.g_SkyColor = ocean_params.g_SkyColor;
	shading_data.g_WaterbodyColor = ocean_params.g_WaterbodyColor;
	shading_data.g_Shineness = ocean_params.g_Shineness;
	shading_data.g_SunDir = ocean_params.g_SunDir;
	shading_data.g_SunColor = ocean_params.g_SunColor;
	shading_data.g_BendParam = ocean_params.g_BendParam;
	shading_data.g_PerlinSize = ocean_params.g_PerlinSize;
	shading_data.g_PerlinAmplitude = ocean_params.g_PerlinAmplitude;
	shading_data.g_PerlinOctave = ocean_params.g_PerlinOctave;
	shading_data.g_PerlinGradient = ocean_params.g_PerlinGradient;
	shading_data.g_TexelLength_x2 = ocean_params.g_PatchLength / ocean_params.g_MapDimension * 2;
	shading_data.g_UVScale = 1.0f / ocean_params.g_PatchLength;
	shading_data.g_UVOffset = 0.5f / ocean_params.g_MapDimension;
	...
	// Map shading constants
	{
		...
		m_pD3DSystem->GetDeviceContext()->Map(m_pShadingCB, 0, D3D11_MAP_WRITE_DISCARD, 0, &mapped_res);
		{
			...
			*(cbShadingBuffer*)mapped_res.pData = shading_data;
		}
		m_pD3DSystem->GetDeviceContext()->Unmap(m_pShadingCB, 0);
	}
	
	ID3D11Buffer* cbs[1] = {m_pShadingCB};
	m_pD3DSystem->GetDeviceContext()->VSSetConstantBuffers(0, 1, cbs);
	m_pD3DSystem->GetDeviceContext()->PSSetConstantBuffers(0, 1, cbs);
}
//==============================================================================================================================
void OceanSurfaceShader::SetPerCallBuffer(OceanParameters ocean_params, XMFLOAT2 bottom_left,
...
{
	cbPerCallBuffer cbPCB;
	
	cbPCB.g_matLocal = local.Transpose().ToFloat4x4();
	
	ZShadeSandboxMath::XMMatrix world;
	world.Translate(bottom_left.x, 0, bottom_left.y);
	ZShadeSandboxMath::XMMatrix wvp = world * view * proj;
	cbPCB.g_matWorldViewProj = wvp.Transpose().ToFloat4x4();
	
	cbPCB.g_matWorld = world.Transpose().ToFloat4x4();
	
	ZShadeSandboxMath::XMMatrix wrvp = world * refView * proj;
	cbPCB.g_matWorldReflectionViewProj = wrvp.Transpose().ToFloat4x4();
	
	// Perlin UV
	XMFLOAT2 uv_base = ZShadeSandboxMath::XMMath2(bottom_left) / ocean_params.g_PatchLength * ocean_params.g_PerlinSize;
	cbPCB.g_UVBase = uv_base;
	
	// Constant m_PerlinSpeed need to be adjusted manually
	...
	cbPCB.g_PerlinMovement = perlin_move;
	
	// eye point
	ZShadeSandboxMath::XMMatrix matInvWV = ZShadeSandboxMath::XMMatrix(world * view).Inverse();
	ZShadeSandboxMath::XMMath3 localEye(0, 0, 0);
	localEye = localEye.Transform(matInvWV);
	cbPCB.g_LocalEye = (XMFLOAT3)localEye;
	
	cbPCB.g_SeaLevel = ocean_params.g_SeaLevel;
	
	// Map per call constants
	{
		D3D11_MAPPED_SUBRESOURCE mapped_res;
		m_pD3DSystem->GetDeviceContext()->Map(m_pPerCallCB, 0, D3D11_MAP_WRITE_DISCARD, 0, &mapped_res);
		{
			assert(mapped_res.pData);
			*(cbPerCallBuffer*)mapped_res.pData = cbPCB;
		}
		m_pD3DSystem->GetDeviceContext()->Unmap(m_pPerCallCB, 0);
	}
	
	ID3D11Buffer* cbs[1] = {m_pPerCallCB};
	m_pD3DSystem->GetDeviceContext()->VSSetConstantBuffers(1, 1, cbs);
	m_pD3DSystem->GetDeviceContext()->PSSetConstantBuffers(1, 1, cbs);
}
//==============================================================================================================================
/*bool OceanSurfaceShader::Render11( int indexCount, Camera* camera, int frustumCull, XMMATRIX world, OceanParameters ocean_params, XMFLOAT2 bottom_left)
{
	...
	cFB.g_LightDirection = ocean_params.g_LightDirection;//XMFLOAT3(0.936016f, -0.343206f, 0.0780013f);
	...
	// Texcoord for perlin noise (not being used at the moment)
	XMFLOAT2 uv_base = ZShadeSandboxMath::XMMath2(bottom_left) / ocean_params.g_PatchLength * ocean_params.g_PerlinSize;
	...
	ID3D11SamplerState* ps_samp[5] = { g_pCubeSampler, g_pPerlinSampler, g_pGradientSampler, g_pFresnelSampler, m_pD3DSystem->Mirror() };
	...
	if (!m_Wireframe)
	{
		...
		m_pD3DSystem->GetDeviceContext()->PSSetSamplers(0, 5, ps_samp);
	...
	if (!m_Wireframe)
	{
		...
		m_pD3DSystem->GetDeviceContext()->PSSetShaderResources(0, 9, ps_srvs);
		for (int i = 0; i < 5; i++) ps_samp[i] = NULL;
		m_pD3DSystem->GetDeviceContext()->PSSetSamplers(0, 5, ps_samp);
	}
