...
bool OceanEnvironment::Init()
{
	...
	ocean_params.g_OceanSize = 4096;//8192 * 2; //<JPK> the relative values of these are critical to "scale" of the FFT effect
	ocean_params.g_PatchSize = 512;
	ocean_params.g_UpperGridCoverage = 64.0f;
	ocean_params.g_FurthestCover = 8.0f;
	ocean_params.g_MinDist = 100.0f;
	ocean_params.g_MaxDist = 1000.0f;
	ocean_params.g_MinTess = 2.0f;
	ocean_params.g_MaxTess = 16.0f;//128.0f;
	ocean_params.g_SeaLevel = fSeaLevel;
	ocean_params.g_SkyColor = XMFLOAT3(0.38f, 0.45f, 0.56f);
	ocean_params.g_WaterbodyColor = XMFLOAT3(0.07f, 0.15f, 0.2f);
	ocean_params.g_Skyblending = 16.0f;
	ocean_params.g_SunDir = XMFLOAT3(0.936016f, -0.343206f, 0.0780013f);
	ocean_params.g_SunColor = XMFLOAT3(1.0f, 1.0f, 0.6f);
	ocean_params.g_Shineness = 400.0f;

	// Max is 256
	ocean_params.g_MeshDimention = 128;
	...
	ocean_params.g_BendParam = XMFLOAT3(0.1f, -0.4f, 0.2f);
	...
	ocean_params.g_PatchLength = 2048;//2000;
...
void OceanEnvironment::Update()
{
	...
	mOceanSurface->Update(fSeaLevel, m_Timer.ConstantTime());
	
	//get a height at a position 512x512 in displacement map
	...
	/*for (int i = 0; i < 2; i++)
	{
		...
		float unit = ocean_params.g_OceanSize / ocean_params.g_MapDimension;
		...
		{
			if (i == 0)
				mCubeAboveWater->Position() = XMFLOAT3(0, 5, 0);
			else
				mCubeBelowWater->Position() = XMFLOAT3(0, -5, 0);
			return;
		}
		
		int ix = (int)(x * ocean_params.g_MapDimension);
		int iz = (int)(z * ocean_params.g_MapDimension);
		...
		for (int x = -2; x < 2; x++)
		{
			for (int z = -2; z < 2; z++)
			{
				XMFLOAT3 wave = mOceanSurface->ReadWave(ix + x, iz + z);
		...
		XMFLOAT3 wave = mOceanSurface->ReadWave(ix, iz);
		
		// minimal bicubic weight
		h = ((wave.y * 3) + h) / 4.0f;
		//h = wave.y;
		
		//set the cube at that height      
		if (i == 0)
		{
...
void OceanEnvironment::Render()
{
	...
	mOceanSurface->Render(m_CameraSystem);
