...
//===============================================================================================================================
void MeshAttributes::BuildVertexBuffer(ZShadeSandboxMesh::EVertexType::Type type, bool dynamic)
{
	switch (type)
	{
		case ZShadeSandboxMesh::EVertexType::Type::VT_Pos: BuildVertexBuffer<ZShadeSandboxMesh::VertexPos>(mVerticesPos, dynamic); break;
		case ZShadeSandboxMesh::EVertexType::Type::VT_Color: BuildVertexBuffer<ZShadeSandboxMesh::VertexColor>(mVerticesColor, dynamic); break;
		case ZShadeSandboxMesh::EVertexType::Type::VT_Tex: BuildVertexBuffer<ZShadeSandboxMesh::VertexTex>(mVerticesTex, dynamic); break;
		case ZShadeSandboxMesh::EVertexType::Type::VT_NormalTex: BuildVertexBuffer<ZShadeSandboxMesh::VertexNormalTex>(mVerticesNormalTex, dynamic); break;
		case ZShadeSandboxMesh::EVertexType::Type::VT_NormalTexTan: BuildVertexBuffer<ZShadeSandboxMesh::VertexNormalTexTan>(mVerticesNormalTexTan, dynamic); break;
		case ZShadeSandboxMesh::EVertexType::Type::VT_NormalTexTanBi: BuildVertexBuffer<ZShadeSandboxMesh::VertexNormalTexTanBiTan>(mVerticesNormalTexTanBi, dynamic); break;
		default:
		{
		   //The shader was not defined
		   ZShadeMessageCenter::MsgBoxError(NULL, "Mesh Vertex Type was not defined !!!");
...
//===============================================================================================================================
void MeshAttributes::SetPrimitiveTopology(ZShadeSandboxMesh::ERenderType::Type renderType)
{
...
//===============================================================================================================================
void MeshAttributes::SetBuffers(bool useIndexBuffer)
{
	unsigned int stride;
	unsigned int offset;
...
//===============================================================================================================================
void MeshAttributes::SetBuffersInstanced(bool useIndexBuffer)
{
...
//===============================================================================================================================
void MeshAttributes::AddInstanceMatrix
...
)
{
	...
	if (instancePositions.size() != instanceRotations.size()
	&&	instancePositions.size() != instanceScales.size()
	&&	instanceRotations.size() != instanceScales.size()
	|| instancePositions.size() == 0
	|| instanceRotations.size() == 0
	|| instanceScales.size() == 0)
	{
		return;
	}
	...
	if (mInstanceMatrix.size() > 0)
	{
		used = true;
		mInstanceMatrix.clear();
	}
	
	mInstanceCount = instancePositions.size();
	mInstanceMatrix.resize(mInstanceCount);
	mInstanceByteWidth = sizeof(ZShadeSandboxMesh::InstanceMatrix);
	
	for (int i = 0; i < mInstanceCount; i++)
	{
		mInstanceMatrix[i].position = instancePositions[i];
		mInstanceMatrix[i].rotation = instanceRotations[i];
		mInstanceMatrix[i].scale = instanceScales[i];
	}
	
	(used) ? ReBuildInstanceBuffer() : BuildInstanceBuffer();
}
//===============================================================================================================================
void MeshAttributes::AddInstanceMatrix(vector<ZShadeSandboxMesh::InstanceMatrix> instMat)
{
	bool used = false;
	...
	mInstanceCount = instMat.size();
	mInstanceMatrix.resize(mInstanceCount);
	mInstanceByteWidth = sizeof(ZShadeSandboxMesh::InstanceMatrix);

	for (int i = 0; i < mInstanceCount; i++)
	{
		mInstanceMatrix[i].position = instMat[i].position;
		mInstanceMatrix[i].rotation = instMat[i].rotation;
		mInstanceMatrix[i].scale = instMat[i].scale;
	}
...
void MeshAttributes::BuildInstanceBuffer()
{
	if (mInstanceMatrix.size() > 0)
	{
		...
		instanceData.pSysMem = &mInstanceMatrix[0];
	...
	{
		// Just create the instance buffer without data
		HRESULT result;
		D3D11_BUFFER_DESC instanceBufferDesc;
		...
		// Create the instance buffer
		result = mD3D->GetDevice11()->CreateBuffer(&instanceBufferDesc, 0, &mInstanceBuffer);
		if (FAILED(result)) return;
	}
...
void MeshAttributes::ReBuildInstanceBuffer()
{
	...
	memcpy(resource.pData, &mInstanceMatrix[0], mInstanceByteWidth * mInstanceCount);
