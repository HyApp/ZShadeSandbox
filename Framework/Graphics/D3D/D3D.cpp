...
//===============================================================================================================================
bool D3D::InitializeAdapterAndModes11()
{
	HRESULT hr;
...
//===============================================================================================================================
bool D3D::Initialize12(EngineOptions* options)
{
	// Initialize the GDI Plus interface to use with DirectX
	m_gdi = new GDI();
	...
	//Initialize the DirectX interface adapters and get display modes
	InitializeAdapterAndModes12();

	//Create the Swap chain and device
	InitializeDeviceAndSwapChain12();

	//Create the backbuffers and render targets
	InitializeBackbuffers12();
	
	//Create the command list and fence for the GPU
	InitializeCommandListAndFence12();
...
//===============================================================================================================================
bool D3D::InitializeAdapterAndModes12()
{
	/*
	HRESULT result;
	uint32 numModes, i, stringLength;
	DXGI_MODE_DESC* displayModeList;
	DXGI_ADAPTER_DESC adapterDesc;
	int error;
	
	// Create a DirectX graphics interface factory.
	result = CreateDXGIFactory1(__uuidof(IDXGIFactory4), (void**)&factory4);
	if(FAILED(result))
	{
		return false;
	}
	...
	// Use the factory to create an adapter for the primary graphics interface (video card).
	result = factory4->EnumAdapters(0, &adapter);
	if(FAILED(result))
	{
		return false;
	}
	...
	for (i = 0; i < numModes; i++)
	{
		if (displayModeList[i].Height == (uint32)m_screenWidth)
		{
			if (displayModeList[i].Width == (uint32)m_screenHeight)
			{
...
//===============================================================================================================================
bool D3D::InitializeDeviceAndSwapChain12()
{
	/*
	D3D_FEATURE_LEVEL featureLevel;
	HRESULT result;
	...
	result = D3D12CreateDevice(NULL, featureLevel, __uuidof(ID3D12Device), (void**)&m_device12);
	if(FAILED(result))
	{
		...
		ZShadeMessageCenter::MsgBoxError(NULL, "Could not create a DirectX 12.1 device.  The default video card does not support DirectX 12.1. Falling back to DirectX 11.");
		
		// If DirectX 12 fails then fall back to DirectX 11
		return Initialize11(mOptions);
	}
	...
	result = m_device12->CreateCommandQueue(&commandQueueDesc, __uuidof(ID3D12CommandQueue), (void**)&m_commandQueue);
	if(FAILED(result))
	{
		return false;
	}
	...
	result = factory4->CreateSwapChain(m_commandQueue, &swapChainDesc, &swapChain);
	if(FAILED(result))
	{
		return false;
	}
	...
	result = swapChain->QueryInterface(__uuidof(IDXGISwapChain3), (void**)&m_swapChain3);
	if(FAILED(result))
	{
		return false;
	}
...
//===============================================================================================================================
bool D3D::InitializeBackbuffers12()
{
	/*
	HRESULT result;
	...
	result = m_device12->CreateDescriptorHeap(&rtvHeapDesc, __uuidof(ID3D12DescriptorHeap), (void**)&m_rtvHeap);
	if(FAILED(result))
	{
		return false;
	}
	...
	rtvDescriptorSize = m_device12->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_RTV);
	
	// Get a pointer to the first back buffer from the swap chain.
	result = m_swapChain3->GetBuffer(0, __uuidof(ID3D12Resource), (void**)&m_backBufferRTV[0]);
	if(FAILED(result))
	{
		return false;
	}
	...
	m_device12->CreateRenderTargetView(m_backBufferRTV[0], NULL, rtvHandle);
	
	// Increment the view handle to the next descriptor location in the render target view heap.
	rtvHandle.ptr += rtvDescriptorSize;
	
	// Get a pointer to the second back buffer from the swap chain.
	result = m_swapChain3->GetBuffer(1, __uuidof(ID3D12Resource), (void**)&m_backBufferRTV[1]);
	if(FAILED(result))
	{
		return false;
	}
	...
	m_device12->CreateRenderTargetView(m_backBufferRTV[1], NULL, rtvHandle);
	
	// Finally get the initial index to which buffer is the current back buffer.
	m_bufferIndex = m_swapChain3->GetCurrentBackBufferIndex();
	*/

	return true;
}
//===============================================================================================================================
bool D3D::InitializeCommandListAndFence12()
{
	/*
	HRESULT result;
	
	// Create a command allocator.
	result = m_device12->CreateCommandAllocator(D3D12_COMMAND_LIST_TYPE_DIRECT, __uuidof(ID3D12CommandAllocator), (void**)&m_commandAllocator);
	if(FAILED(result))
	{
		return false;
	}
	...
	result = m_device12->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT, m_commandAllocator, NULL, __uuidof(ID3D12GraphicsCommandList), (void**)&m_commandList);
	if(FAILED(result))
	{
		return false;
	}
	...
	result = m_device12->CreateFence(0, D3D12_FENCE_FLAG_NONE, __uuidof(ID3D12Fence), (void**)&m_fence);
	if(FAILED(result))
	{
		return false;
	}
...
//===============================================================================================================================
ID3D11DepthStencilView* D3D::GetDepthStencilView() { return m_pBackbufferDS->GetDSView(); }
//===============================================================================================================================
...
void D3D::InitRenderStates()
{
	...
	else if (mOptions->m_d3dVersion == DIRECTX11)
	{
		...
		*/
		...
		sampDesc.BorderColor[0] = sampDesc.BorderColor[1] = sampDesc.BorderColor[2] = sampDesc.BorderColor[3] = 1;
